...
//	printf("PTRACE_SEIZE here\n");
	...
	if (ret) {
		...
		printf("Unable to interrupt task: %d (%s)\n", pid, strerror(errno));
		return ret;
	}
...
int compel_remote_open(struct parasite_ctl *ctl, const char* path, int flags, mode_t mode)
{
	void *where = (void *)ctl->ictx.syscall_ip + BUILTIN_SYSCALL_SIZE;
	uint8_t orig_code[REMOTE_OPEN_SZ];
	pid_t pid = ctl->rpid;
	long sret = -ENOSYS;
	int ret, fd, len, temp;
...
/*
	if(len > REMOTE_OPEN_SZ){
		printf("pathname is too long\n");
		return -1;
	}
*/
	// copy the file path
	memcpy(orig_code, path, len);

	//printf("orig_code: %s\n", orig_code);
	
	temp = len/sizeof(long);
	if(len%sizeof(long))
		temp++;

	len = temp*sizeof(long);
	...
	if (ptrace_swap_area(pid, where, (void *)orig_code, len)) {
		pr_err("Can't inject open args (pid: %d)\n", pid);
		printf("Can't inject open args (pid: %d)\n", pid);
		return -1;
	}

	ret = compel_syscall(ctl, __NR(open, compat_task), &sret,
			     (unsigned long)where, flags, mode, 0, 0, 0);

	// cannot openat at pie
	fd = (int)sret;

	if(fd < 0){
		pr_err("Open file by dumpee (applications to be dumped) fail. The dumpee should have the write permission of the directory to store the checkpoints\n");
		return -1;
	}


	// after open, cope back the original code
	if (ptrace_poke_area(pid, orig_code, where, len)) {
		if (fd >= 0)
	...
	if (fd < 0) {
		errno = -fd;
		pr_perror("Can't open a file in victim");
		return fd;
	}

	pr_info("Open a file in parasite\n");
	return fd;
}
...
static int compel_map_exchange(struct parasite_ctl *ctl, unsigned long size)
{
	...
	if (ret == 1) {
		printf("MemFD parasite doesn't work, goto legacy mmap\n");
...
//	int fd;
	...
	//printf("Putting parasite blob into %p->%p\n", ctl->local_map, ctl->remote_map);
...
static int simple_open_proc(int pid, int mode, const char *fmt, ...)
{
	...
	printf("simple open proc in infect.c\n");
