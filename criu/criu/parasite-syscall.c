...
struct parasite_ctl *parasite_infect_seized(pid_t pid, struct pstree_item *item,
		struct mem_dump_ctl *mdc, struct vm_area_list *vma_area_list)
{
	...
	// open a file in curWD in victim and get fd
	if((opts.use_pmem == true)&&(mdc->pre_dump == false)){
		// only nv-dimm and !predump
		// get path here
		get_pages_image_path(path);
		//printf("pages path: %s\n", path);
		// open file in pie

		fd = compel_remote_open(ctl, path, O_CREAT|O_RDWR|O_TRUNC, 0666);
		//fd = compel_remote_open(ctl, path, O_CREAT|O_RDWR|O_TRUNC, CR_FD_PERM);
	}
